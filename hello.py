...
def index():
    ...
    if form.validate_on_submit():
        ...
        session['email_reminder'] = email_reminder

        return redirect(url_for('index'))

    return render_template('index.html', form=form, name=session.get('name'), email = session.get('email'), email_reminder = session.get('email_reminder'))
